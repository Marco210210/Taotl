...
CREATE OR REPLACE PACKAGE taotl_api AS
  ...
  PROCEDURE sync_game(p_body IN CLOB);
...
CREATE OR REPLACE PACKAGE BODY taotl_api AS
  ...
  PROCEDURE sync_game(p_body IN CLOB) IS
    v_game_id     games.id%TYPE;
    v_mode        games.mode%TYPE;
  ...
  BEGIN
    ...
    ON (g.id = src.id)
    WHEN NOT MATCHED THEN
      INSERT (id, mode, num_players, start_dealer_id, created_at, finished_at)
